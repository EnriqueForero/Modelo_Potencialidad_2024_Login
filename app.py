...
from auth import init_login, create_login_layout, VALID_USERNAME_PASSWORD, User
...
from auth import init_login, create_login_layout
from flask_login import login_required, current_user
from dash.dependencies import Input, Output, State

# Modificar la definición del servidor Flask
server = flask.Flask(__name__, template_folder='templates')
server.config['SECRET_KEY'] = 'exportaciones2024'  # Cambia esto por una clave segura
...
def display_page(pathname):
    if not current_user.is_authenticated:
        # Always return login layout if not authenticated
        return create_login_layout()
    
    # User is authenticated, handle routing
    if pathname == '/paises-potenciales' or pathname == '/paises-potenciales/':
        return get_app_layout()
    elif pathname == '/productos-potenciales' or pathname == '/productos-potenciales/':
        # Return app2 layout directly instead of redirecting
        return app2.layout
    elif pathname == '/metodologia' or pathname == '/metodologia/':
        return flask.redirect('/metodologia')
...
def home():
    ...
    return flask.render_template_string('''
        ...
        <html>
            <head>
                <title>Login - Modelos de Potencialidad</title>
                    ...
                    $(document).ready(function() {
                        $('#login-form').on('submit', function(e) {
                            ...
                            $.ajax({
                                ...
                                error: function(xhr) {
                                    var message = xhr.responseJSON ? xhr.responseJSON.message : 'Error al iniciar sesión';
                                    $('#login-alert').html(
...
def login_post():
    ...
    print(f"Login attempt - Username: {username}")  # Para debugging
...
@server.route("/paises-potenciales/")
@login_required
def paises_potenciales():
    # Return the Dash app layout directly
    return app.index()

@server.route("/productos-potenciales/")
@login_required
def productos_potenciales():
    # Return the Dash app layout directly
    return app2.index()
...
# Initialize app2 with authentication
app2 = dash.Dash(
    __name__,
    server=server,
...
# Initialize login manager for app2
init_login(app2)
